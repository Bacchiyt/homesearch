...
- Phase 1 now includes the Python 3.14/uv bootstrap, strict versioned configuration loading, the explicit one-user/default-user anchor, empty-by-default source/search policy registries, a secret-safe synchronous SQLAlchemy 2/psycopg 3 PostgreSQL engine boundary, a local PostgreSQL 18.4 Compose service, and the first Alembic revision for safe configuration snapshots, user/source identities, separate property/listing anchors, and the user-owned run ledger. SQLAlchemy mappings, repositories/transactions, workflows, structured logging, and CI remain unimplemented.
