...
**Progress:** independently reviewed tasks now provide the Python 3.14/uv package and local quality commands, strict layered configuration loading, the one-user/default-user UUIDv7 anchor, and empty-by-default source/search policy registries. Source policies validate lifecycle/access, capabilities, request limits, and capture/retention/storage vocabulary without authorizing access; search policies validate source-neutral criteria and user/source references. The accepted synchronous SQLAlchemy 2/psycopg 3 connection boundary resolves only a configured database secret, and the local workflow provides a single PostgreSQL 18.4 Compose service with loopback-only access, ignored file-backed credentials, health checking, and named storage. Alembic uses that same configuration/engine boundary. Its first revision now creates safe configuration snapshots, user/source identities, separate property/listing anchors, and a user-owned top-level run ledger with application-owned UUIDs, explicit constraints, UTC-aware timestamps, and migration metadata. No rows are seeded and no search is enabled by default. SQLAlchemy mappings, repositories/transactions, application workflows, source/provider adapters, polling, scheduling/jobs, structured logging, and CI remain separate follow-up tasks.
