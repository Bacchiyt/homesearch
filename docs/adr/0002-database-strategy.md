...
- Target PostgreSQL 18 and follow its [supported current minor release](https://www.postgresql.org/support/versioning/). Pin the development/CI major and production major; do not float across majors implicitly.
...
- PostgreSQL 18 availability may lag on a future host; Gate D must verify host support and current supported minors.
...
- In Phase 1, prove empty-schema migration, upgrade, constraints, transaction behavior, UTC/JSON round trips, and restore into a fresh PostgreSQL 18 instance.
